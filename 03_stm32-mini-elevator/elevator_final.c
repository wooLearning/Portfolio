...
static int has_any_request_at(int idx) {
  if (idx < 0 || idx >= MAXFLOOR) return 0;
  return upButton[idx] || downButton[idx] || fButton[idx];
}

static int has_up_request_at(int idx) {
  if (idx < 0 || idx >= MAXFLOOR) return 0;
  return upButton[idx] || fButton[idx];
}

static int has_down_request_at(int idx) {
  if (idx < 0 || idx >= MAXFLOOR) return 0;
  return downButton[idx] || fButton[idx];
}

int button_check() {
  int currentIdx = currentFloor - 1;
  int i;

  // SCAN 방식:
  // 상승 중에는 UP/내부(F) 우선, DOWN은 상행 처리 후 하행 시 수거
  if (direction == 1) {
    for (i = currentIdx; i < MAXFLOOR; i++) {
      if (has_up_request_at(i)) return i + 1;
    }
    for (i = currentIdx + 1; i < MAXFLOOR; i++) {
      if (downButton[i]) return i + 1;
    }
    for (i = currentIdx; i >= 0; i--) {
      if (has_any_request_at(i)) return i + 1;
    }
    direction = 0;
    return currentFloor;
  }

  // 하강 중에는 DOWN/내부(F) 우선, UP은 하행 처리 후 상행 시 수거
  if (direction == 2) {
    for (i = currentIdx; i >= 0; i--) {
      if (has_down_request_at(i)) return i + 1;
    }
    for (i = currentIdx - 1; i >= 0; i--) {
      if (upButton[i]) return i + 1;
    }
    for (i = currentIdx; i < MAXFLOOR; i++) {
      if (has_any_request_at(i)) return i + 1;
    }
  ...
  // 정지 상태: 가장 가까운 요청 층 선택
  {
    ...
    for (i = 0; i < MAXFLOOR; i++) {
      int floor = i + 1;
      int dist;
      if (!has_any_request_at(i)) continue;

      dist = floor - currentFloor;
      if (dist < 0) dist = -dist;

      if (bestFloor == currentFloor || dist < bestDist) {
        bestFloor = floor;
        bestDist = dist;
      } else if (dist == bestDist && floor > bestFloor) {
        bestFloor = floor;
      }
    }
    return bestFloor;
