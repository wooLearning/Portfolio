...
module tb_SocTopUartLoaderRamTrapCallProbe;
  localparam integer LP_BIT_CLKS = 432;
  localparam integer LP_PACKET_BYTES = 556;
  ...
  initial begin
    $readmemh("../SW/build_outputs/ram_trap_call_probe_loader_packet.hex", rPacket);
  end
  ...
  initial begin
    ...
    repeat (100000) begin
      ...
      if ((wGpioADir == 16'hFFFF) && (wGpioAOut == 16'h55AA) &&
          (wDbgPc >= 32'h20001000)) begin
        $display("SOC_RAM_TRAP_CALL_PROBE_PASS cycle=%0d retired=%0d pc=0x%08h gpio=0x%04h mcause=0x%08h mepc=0x%08h",
                 wDbgCycleCount,
                 wDbgRetiredCount,
                 wDbgPc,
                 wGpioAOut,
    ...
    $display("SOC_RAM_TRAP_CALL_PROBE_TIMEOUT cycle=%0d retired=%0d pc=0x%08h gpio_dir=0x%04h gpio=0x%04h mcause=0x%08h mepc=0x%08h mstatus=0x%08h ibus_wait=%0d dbus_wait=%0d",
             wDbgCycleCount,
             wDbgRetiredCount,
             wDbgPc,
             wGpioADir,
             ...
             wDbgIbusWaitCount,
             wDbgDbusWaitCount);
    $fatal(1, "RAM trap call probe did not return through handler and mret");
  end
endmodule
