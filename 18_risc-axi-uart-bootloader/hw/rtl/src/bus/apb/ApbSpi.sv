...
module ApbSpi #(
  ...
  assign oTxS_TREADY  = rTxDmaEnable && wTxFifoPushReady && !wTxWrite;
  ...
  assign wRxPush      = wSpiDone && wRxFifoPushReady;
  assign oPREADY      = 1'b1;
  ...
  always_ff @(posedge iPclk or negedge iPresetn) begin
    if (!iPresetn) begin
      ...
      if (wRxRead && (wRxFifoCount == {{(LP_CNT_WIDTH-1){1'b0}}, 1'b1}) && !wRxPush) begin
        rRxValidSticky <= 1'b0;
      end
  ...
  ByteFifo #(
    ...
    .iRstn       (iPresetn),
    .iPush       (wTxPush),
    .iPushData   (wTxWrite ? iPWDATA[7:0] : iTxS_TDATA),
