...
/*
[MODULE_INFO_START]
Name: AxiLiteInterconnect1x4
Role: One-master AXI-Lite address decoder for I-SRAM, D-SRAM, DMA, and peripherals
Summary:
  - Routes one AXI-Lite master to four slaves by address window
  - S0: instruction SRAM, S1: data SRAM, S2: DMA control, S3: peripheral control
...
module AxiLiteInterconnect1x4 (
  ...
  output logic [31:0] oS3_AWADDR,
  output logic        oS3_AWVALID,
  input  logic        iS3_AWREADY,
  output logic [31:0] oS3_WDATA,
  output logic [3:0]  oS3_WSTRB,
  output logic        oS3_WVALID,
  input  logic        iS3_WREADY,
  input  logic [1:0]  iS3_BRESP,
  input  logic        iS3_BVALID,
  output logic        oS3_BREADY,
  output logic [31:0] oS3_ARADDR,
  output logic        oS3_ARVALID,
  input  logic        iS3_ARREADY,
  input  logic [31:0] iS3_RDATA,
  input  logic [1:0]  iS3_RRESP,
  input  logic        iS3_RVALID,
  output logic        oS3_RREADY
);

  localparam logic [2:0] LP_SEL_ISRAM  = 3'd0;
  localparam logic [2:0] LP_SEL_DSRAM  = 3'd1;
  localparam logic [2:0] LP_SEL_DMA    = 3'd2;
  localparam logic [2:0] LP_SEL_PERIPH = 3'd3;
  localparam logic [2:0] LP_SEL_ERROR  = 3'd4;
  ...
  function automatic logic [2:0] decode_addr(input logic [31:0] iAddr);
    begin
      ...
      else if (address_map_pkg::in_range(iAddr, address_map_pkg::DMA_BASE, address_map_pkg::DMA_SIZE)) begin
        decode_addr = LP_SEL_DMA;
      end
  ...
  assign oS2_AWADDR = iM_AWADDR - address_map_pkg::DMA_BASE;
  assign oS2_WDATA  = iM_WDATA;
  assign oS2_WSTRB  = iM_WSTRB;
  assign oS2_ARADDR = iM_ARADDR - address_map_pkg::DMA_BASE;
  assign oS3_AWADDR = iM_AWADDR - axi_lite_pkg::PERIPH_BASE;
  assign oS3_WDATA  = iM_WDATA;
  assign oS3_WSTRB  = iM_WSTRB;
  assign oS3_ARADDR = iM_ARADDR - axi_lite_pkg::PERIPH_BASE;
  ...
  assign oS2_AWVALID = iM_AWVALID && (wWriteSel == LP_SEL_DMA);
  assign oS2_WVALID  = iM_WVALID  && (wWriteSel == LP_SEL_DMA);
  assign oS3_AWVALID = iM_AWVALID && (wWriteSel == LP_SEL_PERIPH);
  assign oS3_WVALID  = iM_WVALID  && (wWriteSel == LP_SEL_PERIPH);
  ...
  always_comb begin
    unique case (wWriteSel)
      ...
      LP_SEL_PERIPH: begin
        oM_AWREADY = iS3_AWREADY;
        oM_WREADY  = iS3_WREADY;
      end
      default: begin
  ...
  always_comb begin
    ...
    else if (iS3_BVALID) begin
      oM_BRESP = iS3_BRESP;
      oM_BVALID = iS3_BVALID;
      oS3_BREADY = iM_BREADY;
    end
  ...
  assign oS2_ARVALID = iM_ARVALID && (wReadSel == LP_SEL_DMA);
  assign oS3_ARVALID = iM_ARVALID && (wReadSel == LP_SEL_PERIPH);

  always_comb begin
    unique case (wReadSel)
      LP_SEL_ISRAM:  oM_ARREADY = iS0_ARREADY;
      LP_SEL_DSRAM:  oM_ARREADY = iS1_ARREADY;
      LP_SEL_DMA:    oM_ARREADY = iS2_ARREADY;
      LP_SEL_PERIPH: oM_ARREADY = iS3_ARREADY;
  ...
  always_comb begin
    ...
    else if (iS3_RVALID) begin
      oM_RDATA = iS3_RDATA;
      oM_RRESP = iS3_RRESP;
      oM_RVALID = iS3_RVALID;
      oS3_RREADY = iM_RREADY;
    end
