...
$elf = Join-Path $outDir "uart_dma_spi_rgb_forward.elf"
$bin = Join-Path $outDir "uart_dma_spi_rgb_forward.bin"
$mem = Join-Path $outDir "uart_dma_spi_rgb_forward.mem"
$timingMem = Join-Path $projectRoot "HW\rtl\src\timing_programs\uart_dma_spi_rgb_forward.mem"
...
$map = Join-Path $outDir "uart_dma_spi_rgb_forward.map"
$gccArgs = @(
...
& $objdump -d $elf | Out-File -Encoding ascii (Join-Path $outDir "uart_dma_spi_rgb_forward.dump")

$bytes = (Get-Item $bin).Length
$words = (Get-Content $mem).Count
Write-Host "MASTER_RGB_FORWARD_ROM_BUILD_PASS bytes=$bytes words=$words image_bytes=12288 mem=$mem copied_to=$timingMem"
