param(
  [string]$TimingMemName = "jalr_mret_probe.mem"
)
...
$source = Join-Path $swRoot "firmware_sources\jalr_mret_probe.S"
$linker = Join-Path $swRoot "linker_scripts\linker.ld"
$elf = Join-Path $outDir "jalr_mret_probe.elf"
$bin = Join-Path $outDir "jalr_mret_probe.bin"
$mem = Join-Path $outDir "jalr_mret_probe.mem"
$dump = Join-Path $outDir "jalr_mret_probe.dump"
...
Write-Host "JALR_MRET_PROBE_BUILD_PASS bytes=$bytes words=$words elf=$elf bin=$bin mem=$mem dump=$dump copied_to=$timingMem"
