...
module ControlUnit (
  input  logic [31:0]            iInstr,       // Full instruction for SYSTEM immediate decode.
  ...
  output rv32i_pkg::csr_op_e     oCsrOp,       // Selects CSR read/write operation for SYSTEM instructions.
  output rv32i_pkg::sys_op_e     oSysOp,       // Selects non-CSR SYSTEM operation.
  ...
  always_comb begin
    ...
    if (iInstrValid) begin
      unique case (iOpcode)
        ...
        rv32i_pkg::LP_OPCODE_FENCE: begin
          // FENCE and FENCE.I are treated as ordering no-ops in this
          // single-core, no-cache educational pipeline.
          if ((iFunct3 == 3'b000) || (iFunct3 == 3'b001)) begin
            wDecodeValid = 1'b1;
          end
        ...
        rv32i_pkg::LP_OPCODE_SYSTEM: begin
          unique case (iFunct3)
            3'b000: begin
              unique case (iInstr[31:20])
                12'h000: begin oSysOp = rv32i_pkg::SYS_ECALL;  wDecodeValid = 1'b1; end
                12'h001: begin oSysOp = rv32i_pkg::SYS_EBREAK; wDecodeValid = 1'b1; end
                12'h302: begin oSysOp = rv32i_pkg::SYS_MRET;   wDecodeValid = 1'b1; end
                default: begin end
              endcase
            end
            3'b001: begin oCsrOp = rv32i_pkg::CSR_RW;  wDecodeValid = 1'b1; end
            3'b010: begin oCsrOp = rv32i_pkg::CSR_RS;  wDecodeValid = 1'b1; end
            3'b011: begin oCsrOp = rv32i_pkg::CSR_RC;  wDecodeValid = 1'b1; end
            3'b101: begin oCsrOp = rv32i_pkg::CSR_RWI; wDecodeValid = 1'b1; end
            3'b110: begin oCsrOp = rv32i_pkg::CSR_RSI; wDecodeValid = 1'b1; end
            3'b111: begin oCsrOp = rv32i_pkg::CSR_RCI; wDecodeValid = 1'b1; end
            default: begin end
          endcase

          if (iFunct3 != 3'b000) begin
            oWbSel    = rv32i_pkg::WB_CSR;
            oRegWrite = wDecodeValid;
          end
