...
module tb_SocTopTrapCallProbe;
  ...
  SocTop #(
    .P_ENABLE_DEBUG  (1'b1),
    .P_ICODE_MEM_FILE("rtl/src/timing_programs/trap_call_probe.mem")
  ) uDut (
    .iClk               (rClk),
    .iRstn              (rRstn),
    .iGpioAIn           (16'd0),
  ...
  initial begin
    ...
    repeat (3000) begin
      ...
      if ((wGpioADir == 16'hFFFF) && (wGpioAOut == 16'h55AA)) begin
        $display("SOC_TRAP_CALL_PROBE_PASS cycle=%0d retired=%0d pc=0x%08h gpio=0x%04h mcause=0x%08h mepc=0x%08h",
                 wDbgCycleCount,
                 wDbgRetiredCount,
                 wDbgPc,
                 wGpioAOut,
    ...
    $display("SOC_TRAP_CALL_PROBE_TIMEOUT cycle=%0d retired=%0d pc=0x%08h gpio_dir=0x%04h gpio=0x%04h mtvec=0x%08h mcause=0x%08h mepc=0x%08h mstatus=0x%08h",
             wDbgCycleCount,
             wDbgRetiredCount,
             wDbgPc,
             wGpioADir,
    ...
    $fatal(1, "Trap call probe did not return through handler and mret");
  end
endmodule
