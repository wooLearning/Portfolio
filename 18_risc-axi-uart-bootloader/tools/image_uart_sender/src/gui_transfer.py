...
class TransferGui(tk.Tk):
    def __init__(self) -> None:
        ...
        self.tx_only = tk.BooleanVar(value=False)
        self.expect_inverted = tk.BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(controls, text="TX Only", variable=self.tx_only).grid(
            row=1,
            column=6,
            sticky="w",
            padx=(0, 8),
        )
        ttk.Checkbutton(controls, text="Legacy Raw", variable=self.raw).grid(
            row=1,
            column=7,
            sticky="w",
            padx=(0, 8),
        )
        ttk.Checkbutton(controls, text="Expect Invert", variable=self.expect_inverted).grid(
            row=1,
            column=8,
            sticky="w",
            padx=(0, 8),
        ...
        summary = (
            ...
            "TX Only: send to master and use board LED for result\n"
            "Compare mode: waits for framed RX image from slave\n"
            "Expect Invert: compare against CPU-processed inverted image\n"
            "Legacy Raw: no IMGF header on either side\n"
    ...
    def run_transfer(self) -> None:
        ...
        if self.raw.get():
            ...
            cmd.append("--rx-framed")
        if self.tx_only.get():
            cmd.append("--tx-only")
        if self.expect_inverted.get():
            cmd.append("--expect-inverted")
    ...
    def _drain_output_queue(self) -> None:
        while True:
            ...
            if item == "__PASS__":
                ...
                elif "TX-only: PASS" in text:
                    self.result.set("TX DONE")
                else:
