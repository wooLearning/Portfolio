...
$elf = Join-Path $outDir "uart_dma_sram_invert.elf"
$bin = Join-Path $outDir "uart_dma_sram_invert.bin"
$mem = Join-Path $outDir "uart_dma_sram_invert.mem"
$timingMem = Join-Path $projectRoot "HW\rtl\src\timing_programs\uart_dma_sram_invert.mem"
...
$map = Join-Path $outDir "uart_dma_sram_invert.map"
$gccArgs = @(
  "-march=rv32i_zicsr",
  "-mabi=ilp32",
  "-nostdlib",
...
& $objdump -d $elf | Out-File -Encoding ascii (Join-Path $outDir "uart_dma_sram_invert.dump")

$bytes = (Get-Item $bin).Length
$words = (Get-Content $mem).Count
Write-Host "MASTER_INVERT_ROM_BUILD_PASS bytes=$bytes words=$words mem=$mem copied_to=$timingMem"
