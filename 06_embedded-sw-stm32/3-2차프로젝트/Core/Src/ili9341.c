...
*/
void ILI9341_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t * image) {
	if ((x >= ILI9341_WIDTH) || (y >= ILI9341_HEIGHT)) return;
	if ((x+w-1) >= ILI9341_WIDTH) return;
	if ((y+h-1) >= ILI9341_HEIGHT) return;
	ILI9341_Select();
...
*/
void ILI9341_gameInit(){
	// Draw Borders
	/*
	for(unsigned i=1;i<=2;i++){
		ILI9341_FillRectangle(0, 105*i, ILI9341_WIDTH, 3, ILI9341_WHITE);
	}*/
	/*
	ILI9341_DrawImage(0,0, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(80,0, 75, 105, (uint8_t *)q_image);
	ILI9341_DrawImage(160,0, 75, 105, (uint8_t *)j_image);
	ILI9341_DrawImage(0,107, 75, 105, (uint8_t *)k_image);
	ILI9341_DrawImage(80,107, 75, 105, (uint8_t *)joker_image);
	*/
	ILI9341_DrawImage(X1,Y1, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X2,Y2, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X3,Y3, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X4,Y4, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X5,Y5, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X6,Y6, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X7,Y7, 75, 105, (uint8_t *)back_image);
	ILI9341_DrawImage(X8,Y8, 75, 105, (uint8_t *)back_image);

	ILI9341_WriteString(X0,X0 , "0 try", Font_11x18, ILI9341_WHITE, ILI9341_BLACK);
	//1-5:j  2-7:q  3-6:k  4-8:joker
}
uint8_t ILI9341_cardShape(uint16_t x,uint16_t y){//
	if( (x>X1) && (x<X2) && (y>Y1) && (y<Y3) ){//1
		ILI9341_DrawImage(X1,Y1, 75, 105, (uint8_t *)j_image);
		return 1;
	}
	else if( (x>X2) && (x<ILI9341_WIDTH) && (y>Y2) && (y<Y3) ){//2
		ILI9341_DrawImage(X2,Y2, 75, 105, (uint8_t *)q_image);
		return 2;
	}

	else if( (x>X3) && (x<X4) && (y>Y3) && (y<Y6) ){//3
		ILI9341_DrawImage(X3,Y3, 75, 105, (uint8_t *)k_image);
		return 3;
	}
	else if( (x>X4) && (x<X5) && (y>Y4) && (y<Y6) ){//4
		ILI9341_DrawImage(X4,Y4, 75, 105, (uint8_t *)joker_image);
		return 4;
	}
	else if( (x>X5) && (x<ILI9341_WIDTH) && (y>Y5) && (y<Y6) ){//5
		ILI9341_DrawImage(X5,Y5, 75, 105, (uint8_t *)j_image);
		return 5;
	}
	else if( (x>X6) && (x<X7) && (y>Y6) && (y<ILI9341_HEIGHT) ){//6
		ILI9341_DrawImage(X6,Y6, 75, 105, (uint8_t *)k_image);
		return 6;
	}
	else if( (x>X7) && (x<X8) && (y>Y7) && (y<ILI9341_HEIGHT) ){//7
		ILI9341_DrawImage(X7,Y7, 75, 105, (uint8_t *)q_image);
		return 7;
	}
	else if((x>X8) && (x<ILI9341_WIDTH) && (y>Y8) && (y<ILI9341_HEIGHT) ){//8
		ILI9341_DrawImage(X8,Y8, 75, 105, (uint8_t *)joker_image);
		return 8;
	}else{
		return 0;
	}
}
void ILI9341_cardBack(uint16_t x, uint16_t y){
	ILI9341_DrawImage(x,y, 75, 105, (uint8_t *)back_image);
}
void ILI9341_tryDisplay(int try){
	char str[7];
	sprintf(str, "%d try", try);
	ILI9341_WriteString(X0,X0 , str, Font_11x18, ILI9341_WHITE, ILI9341_BLACK);
}
...
void draw_line_gradation(unsigned line_num, uint8_t * line_buf) {
	...
	uint8_t r=0,g=0,b=0;
	uint8_t bright = (line_num + start_intensity)%32*8;
