...
- **[이슈/문제 상황]**: 카메라 입력, PL 연산부(100MHz), TFT-LCD 출력단(12.5MHz)의 클럭 도메인이 서로 달라 초기 설계에서는 **카메라 프레임 갱신 시점과 LCD 출력 시점이 맞지 않는 sync mismatch**가 발생했고, 단일 메모리 접근 병목까지 겹치며 영상 깨짐(Tearing)과 색상 이상이 나타났습니다.
- **[접근 방식 및 해결]**:
  ...
  - **Sync 동기화 (Latency Hiding)**: LCD의 Read State 동작 중 발생하는 Data 버퍼 오버라이트를 막기 위해 VSYNC 엣지 기반의 read/write 동기화 로직을 적용하고, 2-Stage Synchronizer로 CDC에 의한 Metastability를 예방했습니다. 
- **[결과]**: 카메라 입력과 LCD 출력 간 sync mismatch로 인한 tearing 및 색상 이상을 해결하고, 기존 대비 **약 9.93배 향상된 81.97 FPS의 실시간 고속 영상 병렬 처리**를 안정적으로 달성했습니다.

## 📂 산출물
- **RTL Source**: `RTL/` (Verilog 모듈)
- **Driver Code**: `CdriverCode/` (Vitis 드라이버)
- **Demo**: `3조_FPGA_동작동영상.mp4`
